...
best_pos<-apply(bestfile[,c("prediction","reference")],1,function(x) postive_or_not(x[1],x[2]))
seco_pos<-apply(secondfile[,c("prediction","reference")],1,function(x) postive_or_not(x[1],x[2]))
predata<-t(mapply(c,best_pos,seco_pos))
tabledata<-table(apply(predata,1,function(x) contingency(x[1],x[2],"P")))
tab<-matrix(c(tabledata[4],tabledata[1],tabledata[2],tabledata[3]),byrow = TRUE, 2, 2)
if(fisher.test(tab)$p.value<0.05){
  hightrow[3]<-paste0(names[hight[3]],"*")
}
